...
void control(std::shared_ptr<Optitrack::Human> human,
    ...
    for (int i = 0; i < controller_data.control_links.size(); ++i) {
        ...
        if (controller_data.control_links[i] == "trunk_rz") {
            ...
            // tasks[controller_data.control_links[i]]->setSingularityHandlingGains(20, 20, 0);
    ...
    // create logger 
    // "neck_link2", "trunk_rz", "hip_base", "LL_foot", "RL_foot", "la_end_effector", "ra_end_effector", "la_link4", "ra_link4", "LL_KOSY_L56", "RL_KOSY_L56"
    Vector3d head_pos, torso_pos, hip_pos, left_foot_pos, right_foot_pos, left_hand_pos, right_hand_pos, left_elbow_pos, right_elbow_pos, left_knee_pos, right_knee_pos;
    Vector6d head_vel, torso_vel, hip_vel, left_foot_vel, right_foot_vel, left_hand_vel, right_hand_vel, left_elbow_vel, right_elbow_vel, left_knee_vel, right_knee_vel;
    Matrix3d head_ori, torso_ori, hip_ori, left_foot_ori, right_foot_ori, left_hand_ori, right_hand_ori, left_elbow_ori, right_elbow_ori, left_knee_ori, right_knee_ori;

    // initialize
    VectorXd joint_angles = robot->q();
    VectorXd joint_velocities = robot->dq();

    head_pos       = Eigen::Vector3d::Zero();
    torso_pos      = Eigen::Vector3d::Zero();
    hip_pos        = Eigen::Vector3d::Zero();
    left_foot_pos  = Eigen::Vector3d::Zero();
    right_foot_pos = Eigen::Vector3d::Zero();
    left_hand_pos  = Eigen::Vector3d::Zero();
    right_hand_pos = Eigen::Vector3d::Zero();
    left_elbow_pos = Eigen::Vector3d::Zero();
    right_elbow_pos= Eigen::Vector3d::Zero();
    left_knee_pos  = Eigen::Vector3d::Zero();
    right_knee_pos = Eigen::Vector3d::Zero();

    head_vel        = Eigen::Vector6d::Zero();
    torso_vel       = Eigen::Vector6d::Zero();
    hip_vel         = Eigen::Vector6d::Zero();
    left_foot_vel   = Eigen::Vector6d::Zero();
    right_foot_vel  = Eigen::Vector6d::Zero();
    left_hand_vel   = Eigen::Vector6d::Zero();
    right_hand_vel  = Eigen::Vector6d::Zero();
    left_elbow_vel  = Eigen::Vector6d::Zero();
    right_elbow_vel = Eigen::Vector6d::Zero();
    left_knee_vel   = Eigen::Vector6d::Zero();
    right_knee_vel  = Eigen::Vector6d::Zero();

    head_ori        = Eigen::Matrix3d::Identity();
    torso_ori       = Eigen::Matrix3d::Identity();
    hip_ori         = Eigen::Matrix3d::Identity();
    left_foot_ori   = Eigen::Matrix3d::Identity();
    right_foot_ori  = Eigen::Matrix3d::Identity();
    left_hand_ori   = Eigen::Matrix3d::Identity();
    right_hand_ori  = Eigen::Matrix3d::Identity();
    left_elbow_ori  = Eigen::Matrix3d::Identity();
    right_elbow_ori = Eigen::Matrix3d::Identity();
    left_knee_ori   = Eigen::Matrix3d::Identity();
    right_knee_ori  = Eigen::Matrix3d::Identity();

	Sai2Common::Logger logger("human_optitrack_data", false);
    ...
    // position log
	logger.addToLog(head_pos, "head_pos");
	logger.addToLog(torso_pos, "torso_pos");
    logger.addToLog(hip_pos, "hip_pos");
	logger.addToLog(left_foot_pos, "left_foot_pos");
    logger.addToLog(right_foot_pos, "right_foot_pos");
	logger.addToLog(left_hand_pos, "left_hand_pos");
    logger.addToLog(right_hand_pos, "right_hand_pos");
	logger.addToLog(left_elbow_pos, "left_elbow_pos");
    logger.addToLog(right_elbow_pos, "right_elbow_pos");
	logger.addToLog(left_knee_pos, "left_knee_pos");
    logger.addToLog(right_knee_pos, "right_knee_pos");
    // velocity log
    logger.addToLog(head_vel, "head_vel");
	logger.addToLog(torso_vel, "torso_vel");
    logger.addToLog(hip_vel, "hip_vel");
	logger.addToLog(left_foot_vel, "left_foot_vel");
    logger.addToLog(right_foot_vel, "right_foot_vel");
	logger.addToLog(left_hand_vel, "left_hand_vel");
    logger.addToLog(right_hand_vel, "right_hand_vel");
	logger.addToLog(left_elbow_vel, "left_elbow_vel");
    logger.addToLog(right_elbow_vel, "right_elbow_vel");
	logger.addToLog(left_knee_vel, "left_knee_vel");
    logger.addToLog(right_knee_vel, "right_knee_vel");
    // orientation log 
    logger.addToLog(head_ori, "head_ori");
    logger.addToLog(torso_ori, "torso_ori");
    logger.addToLog(hip_ori, "hip_ori");
	logger.addToLog(left_foot_ori, "left_foot_ori");
    logger.addToLog(right_foot_ori, "right_foot_ori");
	logger.addToLog(left_hand_ori, "left_hand_ori");
    logger.addToLog(right_hand_ori, "right_hand_ori");
	logger.addToLog(left_elbow_ori, "left_elbow_ori");
    logger.addToLog(right_elbow_ori, "right_elbow_ori");
	logger.addToLog(left_knee_ori, "left_knee_ori");
    logger.addToLog(right_knee_ori, "right_knee_ori");
	...
	while (runloop) {
        ...
        if (state == INIT) {
            ...
            for (int i = 0; i < controller_data.control_links.size(); ++i) {
                ...
                // logger variables update 
                // head_pos, torso_pos, hip_pos, left_foot_pos, right_foot_pos, left_hand_pos, right_hand_pos, left_elbow_pos, right_elbow_pos, left_knee_pos, right_knee_pos;
                // "hip_base", "LL_foot", "RL_foot", "trunk_rz", "ra_end_effector", "la_end_effector", "neck_link2", "ra_link4", "la_link4"
                if (i == 0) {
                    hip_pos = tmp_current_pose.translation();
                    hip_ori = tmp_current_pose.linear();
                    hip_vel = tmp_current_vel;
                } else if (i == 1) {
                    left_foot_pos = tmp_current_pose.translation();
                    left_foot_ori = tmp_current_pose.linear();
                    left_foot_vel = tmp_current_vel;
                } else if (i == 2) {
                    right_foot_pos = tmp_current_pose.translation();
                    right_foot_ori = tmp_current_pose.linear();
                    right_foot_vel = tmp_current_vel;
                } else if (i == 3) {
                    torso_pos = tmp_current_pose.translation();
                    torso_ori = tmp_current_pose.linear();
                    torso_vel = tmp_current_vel;
                } else if (i == 4) {
                    right_hand_pos = tmp_current_pose.translation();
                    right_hand_ori = tmp_current_pose.linear();
                    right_hand_vel = tmp_current_vel;
                } else if (i == 5) {
                    left_hand_pos = tmp_current_pose.translation();
                    left_hand_ori = tmp_current_pose.linear();
                    left_hand_vel = tmp_current_vel;
                } else if (i == 6) {
                    head_pos = tmp_current_pose.translation();
                    head_ori = tmp_current_pose.linear();
                    head_vel = tmp_current_vel;
                } else if (i == 7) {
                    right_elbow_pos = tmp_current_pose.translation();
                    right_elbow_ori = tmp_current_pose.linear();
                    right_elbow_vel = tmp_current_vel;
                } else if (i == 8) {
                    left_elbow_pos = tmp_current_pose.translation();
                    left_elbow_ori = tmp_current_pose.linear();
                    left_elbow_vel = tmp_current_vel;
                } 
                // else if (i == 9) {
                //     left_knee_pos = tmp_current_pose.translation();
                //     left_knee_ori = tmp_current_pose.linear();
                //     left_knee_vel = tmp_current_vel;
                // } else if (i == 10) {
                //     right_knee_pos = tmp_current_pose.translation();
                //     right_knee_ori = tmp_current_pose.linear();
                //     right_knee_vel = tmp_current_vel;
                // }
